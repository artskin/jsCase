                ...
                >
                  <div class="popcon" v-if="n.images && n.images.length >0">
                    ...
                    <p class="device-pop-info" v-if="n.images[0].dateTime" :title="n.deviceId+':'+n.deviceName">
                      ...
                      <br>
                      <el-button type="text" @click="openPic(n)" size="mini">查看大图 <i class="el-icon-view"></i></el-button>
                    </p>
...
export default class LocusAnalysis extends Vue {
  ...
  mounted(){
    ...
    //console.log('父级参数',this.parentParams,)
  ...
  openDevicePopover(item,indexN,index){
    ...
    //FIXME:点击后改变当前正在播放的轨迹点的位置
    //FIXME:轨迹到达设备点位时Pop自动弹出（到达下一个点位时，上一个关闭，当前的弹出）
    //FIXME:用户查看某个设备，改变轨迹起始位置后，再播放时，设备POP不弹出问题
  ...
  pOpen(id,autoClose?){
    let _this = this
    console.log('查看设备',id,this.$refs)

    //FIXME: 手动查看设备，关闭后，再播放轨迹时，点位到达下一个设备，不再弹出问题
    ...
    if(this.locusType === "face"){
      ...
      request.getFaceTrackApi(locusParmas).then((resp:any)=>{
        ...
        if(resp && resp.mapVos && resp.mapVos.length && resp.mapVos.length > 0){
          ...
          // this.isExport = false
          // this.hasData = false
          //this.mapList = JSON.parse(JSON.stringify(mockMapData.mapVos)) //mock数据
        }
        //console.log(this.mapList)
        ...
        if(this.deviceMap.size>0 && this.deviceMap.get(0)){
          ...
          //console.log("第一组",this.currentObj.snapshots)
        }
        setTimeout(()=>{
          self.isLoad = false;
  ...
  pathInit(){
    console.log('轨迹路径数据',this.pathArray)
      ...
      self.animationLine.add({
        ...
        changeComplete(e) {
          if(self.deviceMap.get(index)){
            ...
            console.log('设备:',index,self.deviceMap.get(index))
